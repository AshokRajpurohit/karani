...
public class EpicCodeC {
    ...
    public void solve() throws IOException {
        ...
        int n = in.readInt();
        int k = in.readInt();
        int[] boys = in.readIntArray(n);
        int[] girls = in.readIntArray(n);
        sort(boys);
        sort(girls);
        int count = 0;
        int b = 0, g = 0;
        while (b < n && g < n) {
            if (Math.abs(boys[b] - girls[g]) <= k) {
                count++;
                b++;
                g++;
