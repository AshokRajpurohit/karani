...
public class W17_CountFriday {
    ...
    public static void main(String[] args) throws IOException {
        ...
        W17_CountFriday a = new W17_CountFriday();
        a.solve();
        out.close();
    }
    ...
    public void solve() throws IOException {
        ...
        while (t > 0) {
            t--;
            System.out.println(process(in.readInt(), in.readInt(),
                                       in.readInt(), in.readInt(),
                                       in.readInt(), in.readInt()));
        }
        // uncomment the following if you want to print the day of any date.
        /*
        while (true) {
            int d = in.readInt(); // date
            int m = in.readInt(); // month 1 for January, ..., 12 for December
            int y = in.readInt(); // year
            System.out.println(getDay(day(y, m, d)));
        }
        */
    ...
    private static int process(int d1, int m1, int y1, int d2, int m2,
        ...
        if (y1 < y2) {
            ...
            while (m1 <= 12) {
                if (isFriday(y1, m1, 13))
                    count++;
                m1++;
            }

            while (m2 > 0) {
                if (isFriday(y2, m2, 13))
                    count++;
                m2--;
            }
    ...
    private static boolean isFriday(int year, int month, int date) {
        return day(year, month, date) == 5;
    }
