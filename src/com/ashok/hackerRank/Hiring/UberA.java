...
public class UberA {
    ...
    public void solve() throws IOException {
        ...
        int[] res =
            countGreaterNumbers(in.readIntArray(6), in.readIntArray(6));
        for (int e : res)
            out.println(e);
    }

    private static int[] countGreaterNumbers(int[] a, int[] b) {
        if (a.length == 1)
            return new int[] { 0 };

        int[] res = new int[a.length], ref = copy(a);
        Arrays.sort(a);
        for (int i = 0; i < b.length; i++) {
            res[i] = getCount(a, ref[b[i] - 1]);
        }
