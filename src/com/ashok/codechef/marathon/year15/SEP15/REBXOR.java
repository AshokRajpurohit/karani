...
class REBXOR {
    ...
    private static void add(Trie root, int n) {
        int mapper = REBXOR.mapper;
        while (mapper > 0) {
            if ((n & mapper) == 0) {
                if (root.zero == null) {
                    root.zero = new Trie(n, mapper >>> 1);
                    return;
                }
                root = root.zero;
            } else {
                if (root.one == null) {
                    root.one = new Trie(n, mapper >>> 1);
                    return;
                }
                root = root.one;
            }
    ...
    final static class Trie {
        ...
        int query(int n) {
            if ((n & mapper) == 0) {
                if (one != null)
                    return mapper + one.query(n, mapper >>> 1);
                return zero.query(n, mapper >>> 1);
            }

            if (zero != null)
                return mapper + zero.query(n, mapper >>> 1);
