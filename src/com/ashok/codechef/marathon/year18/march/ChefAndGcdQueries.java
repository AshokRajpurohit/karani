...
class ChefAndGcdQueries {
    ...
    private static void populate() {
        ...
        out.println("populated arrays");
        out.flush();
//        preprocessingLock.release();
    }
    ...
    private static void play() throws IOException {
        ...
        while (true) {
            try {
                ...
                int[] ar = Generators.generateRandomIntegerArray(n, 1, LIMIT);
                QueryProcessor processor = getQueryProcessor(ar);
                QueryProcessor brute = new BruteForceQuery(ar);
                int[] ty = Generators.generateRandomIntegerArray(q, 1, 2);
                int[] left = Generators.generateRandomIntegerArray(q, 0, n - 1);
                int[] right = Generators.generateRandomIntegerArray(q, 0, n - 1);
                int[] val = Generators.generateRandomIntegerArray(q, 1, LIMIT);
                Query[] queries = new Query[q];
                for (int i = 0; i < q; i++) {
//                    Query query = new Query(ty[i]);
                    Query query = new Query(2);
                    int le = Math.min(left[i], right[i]);
                    int ri = Math.max(left[i], right[i]);
                    if (query.queryType == QueryType.READ)
                        query.setParams(le, ri, val[i]);
                    else
                        query.setParams(le, ri);

                    queries[i] = query;
                }

                out.println("waiting for the processing to over");
                out.flush();
//                processor.waitForProcessing();
                out.println("waiting for the processing is over");
                out.flush();
                StringBuilder sb = new StringBuilder(q << 2);
                for (Query query : queries) {
                    if (query.queryType == QueryType.READ) {
                        int actual = processor.query(query.a, query.b, query.c);
                        int expected = brute.query(query.a, query.b, query.c);
                        if (expected != actual) {
                            for (int e : ar)
                                sb.append(e).append(' ');
                            ...
                            for (Query qu : queries) {
                                sb.append(2).append(' ');
                                sb.append(qu.a).append(' ');
                                sb.append(qu.b).append(' ');
                                sb.append(qu.c).append('\n');
                            }
                            out.println("Different answer for query: " + query.a + " " + query.b + " " + query.c);
                            out.println("actual: " + actual + ", expected" + expected);
                            out.println(n + " " + q);
                            out.print(sb);
                            out.flush();
                            throw new RuntimeException("lo bhai");
                        }
                    } else {
                        processor.update(query.a, query.b);
                        brute.update(query.a, query.b);
                    }
//                    else
//                        processor.update(query.a, query.b);
                }
...
//                out.print(sb);
                ...
                preprocessingLock = new Semaphore(0);
                processingLock = new Semaphore(0);
            }
        }
    }
    ...
    private static int removeFactor(int n, int f) {
        while (n % f == 0) n /= f;
        return n;
    }
    ...
    private static List<Integer> primeFactors(int n) {
        ...
        while (n > 1) {
            int factor = LARGEST_PRIME_FACTOR[n];
            n = removeFactor(n, factor);
            primeFactors.addFirst(factor);
        }
    ...
    private static void startProcessing(QueryProcessor processor) {
        new Thread(processor::startProcessing).start();
    }

    private static QueryProcessor getQueryProcessor(int[] ar) {
        QueryProcessor queryProcessor = new AvlTreeQueryProcessor(ar);
        queryProcessor.preprocess();
//        startProcessing(queryProcessor);
