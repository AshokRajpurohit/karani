 ...
 * https://www.codechef.com/COOK60/problems/SUMPAIR
 */
...
public class C60B {
    ...
    public void solve() throws IOException {
        ...
        while (t > 0) {
            ...
            int n = in.readInt();
            int d = in.readInt();
            sb.append(process(in.readIntArray(n), d)).append('\n');
        }
        out.print(sb);
    }

    private static long process(int[] ar, int d) {
        sort(ar);
        long res = 0;
        for (int i = ar.length - 1; i > 0; ) {
            if (ar[i] - ar[i - 1] < d) {
                res += ar[i] + ar[i - 1];
                i -= 2;
            } else
