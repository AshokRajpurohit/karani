...
public class Quadtree {
    ...
    public int query(int sr, int er, int sc, int ec) {
    ...
    private static int query(Node node, int sr, int er, int sc, int ec) {
        if (node.startRow == sr && node.endRow == er && node.startCol == sc &&
            node.endCol == ec)
            return node.value;
        ...
        if (sr <= node.midRow && sc <= node.midCol) {
            v1 =
 query(node.first, sr, Math.min(er, node.midRow), sc, Math.min(ec,
                                                               node.midCol));
        }

        if (sr <= node.midRow && ec > node.midCol) {
            v2 =
 query(node.second, sr, Math.min(er, node.midRow), Math.max(sc, node.midCol +
                                                            1), ec);
        }

        if (er > node.midRow && ec > node.midCol) {
            v3 =
 query(node.third, Math.max(sr, node.midRow + 1), er, Math.max(sc,
                                                               node.midCol +
                                                               1), ec);
        }
        ...
        if (er > node.midRow && sc <= node.midCol) {
            v4 =
 query(node.fourth, Math.max(sr, node.midRow + 1), er, sc, Math.min(ec,
                                                                    node.midCol));
        }
    ...
    private static void validatePoint(Node node, int x, int y) {
        if (x < node.startRow || x > node.endRow || y < node.startCol ||
            y > node.endCol)
    ...
    private void populate(Node node) {
        ...
        node.first =
                getNode(node.startRow, node.midRow, node.startCol, node.midCol);
        node.second =
                getNode(node.startRow, node.midRow, node.midCol + 1, node.endCol);
        node.third =
                getNode(node.midRow + 1, node.endRow, node.midCol + 1, node.endCol);
        node.fourth =
                getNode(node.midRow + 1, node.endRow, node.startCol, node.midCol);
     ...
     * @return
     */
    private static int operation(int a, int b, int c, int d) {
        return a + b + c + d;
    }
